...
nePLSret <- paste0('~/R/Data/', QEAperd, '-', modeltype, '-PLSgrpcoef','.csv')
write.csv(round(PLScoef,4), file =nePLSret, row.names = F)
...
if(nrow(PLScoef==3)) {
  for (i in unique(stkdat$Stkcd)) {
    ...
    stkeve <- subset(stkeve, select=charFF3)
  ...
  for (i in unique(stkdat$Stkcd)) {
    stkeve <- filter(stkdat, Stkcd==i)
    stkeve <- subset(stkeve, select=charFF5)
  ...
  for (i in unique(stkdat$Stkcd)) {
    stkeve <- filter(stkdat, Stkcd==i)
    stkeve <- subset(stkeve, select=charCAPM)
    stkeve[,c(2:3)] <- scale(subset(stkeve,select= -Stkcd),center=T,scale=F)
    stkdem <- rbind(stkdem,stkeve)
  }
}
nedem <- paste0('~/R/Data/', QEAperd, '-', modeltype, '-demean', '.csv')
write.csv(stkdem, file=nedem, quote=F, row.names = F)
stkdem <- as.data.frame(read_delim(nedem, delim=',', na = ''))
...
##################################################
timeline <- c(-20:+40)
stkevee <- datwind(2) # event window, key
stkeve <- stkevee[[1]]; windlen <- stkevee[[2]]
if(ncol(OLScoef)==5) {
